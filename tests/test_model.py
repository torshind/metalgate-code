...
@pytest.mark.asyncio
async def test_extract_param_identifier(tmp_path: Path):
    """Test extracting simple identifier parameters."""
    src = b"def f(a, b): pass"
    _, funcs, _, _ = await aparse_file(_file_from_src(src, "mod", tmp_path), [], None)
    assert len(funcs) == 1
    assert [p.name for p in funcs[0].parameters] == ["a", "b"]


@pytest.mark.asyncio
async def test_extract_param_typed(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_extract_param_star(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_extract_param_positional_only(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_extract_docstring(tmp_path: Path):
    """Test extracting function docstrings."""
    _, funcs, _, _ = await aparse_file(
        _file_from_src(b'\ndef f():\n    """hello"""\n    pass\n', "mod", tmp_path),
        [],
        None,
    )
...
@pytest.mark.asyncio
async def test_extract_module_docstring(tmp_path: Path):
    """Test extracting module-level docstrings."""
    md, _, _, _ = await aparse_file(
...
@pytest.mark.asyncio
async def test_extract_exports(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_extract_class_attributes(tmp_path: Path):
    ...
    _, _, classes, _ = await aparse_file(_file_from_src(src, "mod", tmp_path), [], None)
...
@pytest.mark.asyncio
async def test_has_args_kwargs(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_detect_forwarding(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_call_forwards(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_detect_decorator_apps(tmp_path: Path):
    """Test detection of @overload decorator applications."""
    src = "from typing import overload\n@overload\ndef foo(x: int) -> int: ...\n"
    _, _, _, apps = await aparse_file(
        _file_from_src(src.encode(), "mod", tmp_path), [], None
    )
    assert len(apps) == 1
...
@pytest.mark.asyncio
async def test_detect_decorator_apps_multiple(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_detect_decorator_apps_nested_class(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_detect_decorator_apps_with_args(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_detect_decorator_apps_classmethod(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_detect_decorator_apps_property(tmp_path: Path):
...
@pytest.mark.asyncio
async def test_collect_files(tmp_pkg: Path):
    """Test collecting Python files from site-packages."""
    files = await acollect_files(None, [str(tmp_pkg)])
...
@pytest.mark.asyncio
async def test_find_site_packages():
    """Test discovery of site-packages directories."""
    roots = await afind_site_packages(None)
    assert len(roots) > 0
    assert any("site-packages" in r for r in roots)
...
@pytest.mark.asyncio
async def test_parse_file(fake_site: list[Path]):
...
@pytest.mark.asyncio
async def test_parse_file_core_class(fake_site: list[Path]):
